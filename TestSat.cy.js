        ...
        cy.get('[class="datagrid-btable"]').find('tr').each(($el,index,$list)=>{
            ...
            if(textSat.includes(yourSatelliteChoice)){
                ...
                if(countWords(textSat)>1){
                    cy.log('***WARNING: Please use more detailed text to specify "Your Satellite Choice"***')
                    cy.log('An incorrect Satellite selection (number or name) may have been performed due to two or more possible options. Check/uncheck selections from Cypress and resume or re-run test with more detailed text.')
                    cy.log('***Press RESUME if/when selection is correct and verified.***').pause()
                }
